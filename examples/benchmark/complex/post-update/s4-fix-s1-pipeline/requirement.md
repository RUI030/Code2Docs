...
### ac:3: editForm should be updated when editing an existing post
**Given:** The route data contains an existing post record
**When:** The screen initializes
**Then:**
- The editForm should be updated with the existing post's data — title, content, date, blog, and tags are populated into the form fields; `blogsSharedCollection` and `tagsSharedCollection` are also updated to include the post's associations
**Covered by:** test:3
...
### ac:7: compareBlog should forward to blogService
**Given:** The blog dropdown needs to determine whether an option matches the selected value
**When:** The dropdown compares two blog objects
**Then:**
- The compareBlog method should forward to blogService — it forwards both arguments to blogService.compareBlog and returns its result without modification
**Covered by:** test:7

### ac:8: compareTag should forward to tagService
**Given:** The tags multi-select needs to determine whether an option matches a selected value
**When:** The multi-select compares two tag objects
**Then:**
- The compareTag method should forward to tagService — it forwards both arguments to tagService.compareTag and returns its result without modification
**Covered by:** test:8
